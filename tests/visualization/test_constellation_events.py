...
@pytest.mark.asyncio
async def test_started_event_reports_existing_device_assignments():
    """Report assignments already stored on tasks when no mapping is passed."""
    event_bus = AsyncMock()
    orchestrator = TaskConstellationOrchestrator(
        device_manager=AsyncMock(), event_bus=event_bus
    )
    constellation = TaskConstellation("assignment_display_test")
    ...
    event = await orchestrator._start_constellation_execution(
        constellation, device_assignments=None, assignment_strategy=None
    )

    assert event.data["device_assignments"] == {
        "task-1": "windows_device_1"
    }
