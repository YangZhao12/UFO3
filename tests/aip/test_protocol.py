...
class TestHeartbeatProtocol:
    ...
    async def test_send_heartbeat(self):
        ...
        await protocol.send_heartbeat(
            client_id="test_constellation",
            client_type=ClientType.CONSTELLATION,
        )

        assert len(transport.sent_messages) == 1
        sent_message = ClientMessage.model_validate_json(
            transport.sent_messages[0].decode()
        )
        assert sent_message.type == ClientMessageType.HEARTBEAT
        assert sent_message.client_type == ClientType.CONSTELLATION
