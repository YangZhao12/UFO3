...
class UFOWebSocketClient:
    ...
    def __init__(
        ...
        :param api_key: API key required by the UFO WebSocket server
        """
        self.ws_url = ws_url
        self._connection_url = self._with_api_key(ws_url, api_key)
    ...
    @staticmethod
    def _with_api_key(ws_url: str, api_key: Optional[str]) -> str:
        if not api_key:
            return ws_url

        parsed_url = urlsplit(ws_url)
        query = [
            (key, value)
