...
_SETTINGS_URI_COMMAND = re.compile(
    r"^(?:start|start-process)\s+['\"]?(ms-settings:[a-z0-9-]+)['\"]?$",
    re.IGNORECASE,
)


def is_settings_uri_command(command_str: str) -> bool:
    """Return whether the command is exactly an allowed Settings URI launch."""
    return _SETTINGS_URI_COMMAND.fullmatch(command_str.strip()) is not None


def get_settings_uri_page(command_str: str) -> str | None:
    """Return the page portion of an exact Settings URI launch command."""
    match = _SETTINGS_URI_COMMAND.fullmatch(command_str.strip())
    return match.group(1).partition(":")[2].lower() if match else None


def _parse_allowed_command(command_str: str) -> List[str] | None:
    settings_match = _SETTINGS_URI_COMMAND.fullmatch(command_str.strip())
    if settings_match:
        return ["explorer.exe", settings_match.group(1)]
...
def create_cli_mcp_server(*args, **kwargs) -> FastMCP:
    ...
    def run_shell(
        ...
        To open a Windows Settings page, call this tool directly with a command
        such as ``start ms-settings:sound``. Do not launch PowerShell first.
        ...
        args = _parse_allowed_command(bash_command)
        if args is None:
            raise ToolError(
