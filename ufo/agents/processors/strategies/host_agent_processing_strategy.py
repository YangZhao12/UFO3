    ...
    async def execute(
        ...
        try:
            ...
            response_text, llm_cost = await self._get_llm_response_with_retry(
                host_agent, prompt_message, session_step
            )
        ...
        for retry_count in range(max_retries):
            ...
            try:
                ...
                log_phase_event(
                    ...
                    attempt=retry_count + 1,
                    status="success",
                    duration_ms=round((time.perf_counter() - llm_start) * 1000, 3),
                )
...
class HostActionExecutionStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        try:
            ...
            action_status = (
                execution_result[0].status.value if execution_result else "none"
            )
            log_phase_event(
                self.logger,
                "action_end",
                round=context.get("session_step", 0),
                action=function_name,
                status=action_status,
                duration_ms=round((time.perf_counter() - action_start) * 1000, 3),
            )
...
def _is_successful_settings_launch(
    ...
    """Recognize a successful, exact Settings URI launch action."""
    command = (arguments or {}).get("bash_command")
    return bool(
        function_name == "run_shell"
        and isinstance(command, str)
        and is_settings_uri_command(command)
        and execution_result
        and execution_result[0].status == ResultStatus.SUCCESS
    )
...
async def _verify_settings_page(
    ...
    selection_result = await command_dispatcher.execute_commands(
        [
            Command(
                tool_name="select_application_window",
                parameters={
                    "id": str(settings_window["id"]),
