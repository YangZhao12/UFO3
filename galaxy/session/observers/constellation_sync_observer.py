...
class ConstellationModificationSynchronizer(IEventObserver):
    ...
    def complete_modifications(self, task_ids: List[str]) -> None:
        """Complete pending modification waits for the specified tasks."""
        for task_id in task_ids:
            future = self._pending_modifications.pop(task_id, None)
            if future is None:
                continue
            if not future.done():
                future.set_result(True)
                self._stats["completed_modifications"] += 1
            self.logger.info(
                f"Completed modification for task '{task_id}' without constellation editing"
            )
