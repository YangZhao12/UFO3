...
class ConstellationDeviceManager:
    ...
    async def assign_task_to_device(
        ...
        effective_timeout = timeout or self.default_task_timeout
        ...
        task_request.metadata["_deadline"] = deadline
        ...
        if self.device_registry.is_device_busy(device_id):
            ...
            try:
                ...
                self.task_queue_manager.cancel_task(device_id, task_id)
                raise
        else:
    ...
    async def _process_next_queued_task(self, device_id: str) -> None:
        ...
        :param device_id: Device ID
        """
        while self.task_queue_manager.has_queued_tasks(device_id):
            next_task = self.task_queue_manager.dequeue_task(device_id)
            if next_task:
                deadline = next_task.metadata.pop("_deadline", None)
                if deadline is not None:
                    remaining = deadline - asyncio.get_running_loop().time()
                    if remaining <= 0:
                        self.task_queue_manager.fail_task(
                            device_id,
                            next_task.task_id,
                            asyncio.TimeoutError("Task expired while queued"),
                        )
                        continue
