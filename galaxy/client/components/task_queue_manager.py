...
class TaskQueueManager:
    ...
    def cancel_task(self, device_id: str, task_id: str) -> bool:
        """Remove a queued task and its pending future."""
        removed = False
        queue = self._task_queues.get(device_id)
        if queue is not None:
            retained = deque(task for task in queue if task.task_id != task_id)
            removed = len(retained) != len(queue)
            self._task_queues[device_id] = retained

        pending = self._pending_tasks.get(device_id, {})
        future = pending.pop(task_id, None)
        ...
        if removed:
            self.logger.info(
                f"Cancelled queued task {task_id} for device {device_id}"
            )
        return removed
