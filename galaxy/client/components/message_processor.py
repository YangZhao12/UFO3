    ...
    async def _process_server_message(
        ...
        try:
            ...
            if server_msg.type == ServerMessageType.TASK_END:
                ...
                task_error_handled = self.connection_manager.complete_task_error(
                    device_id, server_msg
                )
                # Check if this is a registration error response
                if not task_error_handled:
    ...
    async def _handle_task_completion(
        ...
        try:
            ...
            log_phase_event(
                ...
                device_id=device_id,
                status=(
                    server_msg.status.value
                    if hasattr(server_msg.status, "value")
                    else str(server_msg.status)
                ),
            )
