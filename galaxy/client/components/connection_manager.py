...
class WebSocketConnectionManager:
    ...
    async def send_task_to_device(
        ...
        try:
            ...
            self._pending_tasks[constellation_task_id] = (device_id, task_future)
    ...
    async def _wait_for_task_response(
        ...
        """
        pending_task = self._pending_tasks.get(task_id)
        if pending_task is None:
            task_future = asyncio.get_running_loop().create_future()
            self._pending_tasks[task_id] = (device_id, task_future)
        else:
            pending_device_id, task_future = pending_task
            if pending_device_id != device_id:
                raise ValueError(
                    f"Task {task_id} is already pending for device {pending_device_id}"
                )
    ...
    def complete_task_error(
        self, device_id: str, response: ServerMessage
    ) -> bool:
        """Resolve a pending task when the server returns an ERROR message."""
        task_id = response.session_id
        if task_id is None:
            matching_tasks = [
                pending_task_id
                for pending_task_id, (pending_device_id, _) in self._pending_tasks.items()
                if pending_device_id == device_id
            ]
            if len(matching_tasks) != 1:
                self.logger.warning(
                    f"⚠️ Cannot correlate unscoped error from {device_id} to "
                    f"a pending task (matches: {len(matching_tasks)})"
                )
                return False
        ...
        self.complete_task_response(task_id, response, sender_device_id=device_id)
        return True
