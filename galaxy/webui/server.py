...
async def root() -> HTMLResponse:
    ...
    The runtime API key is embedded for the bundled frontend, which is served
    from the same loopback-only server by default.
    ...
    if frontend_index.exists():
        ...
        app_state = get_app_state()
        api_key_script = (
            "<script>window.__GALAXY_API_KEY__ = "
            f"{json.dumps(app_state.api_key or '')};</script>"
        )
        content = content.replace("</head>", f"{api_key_script}\n  </head>", 1)
